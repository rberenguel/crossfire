...
const AI = {
  ...
  getBestOrientation(aiPlayer, target) {
    ...
    const angleMap = [
      { angle: 0, dir: 0 }, { angle: -45, dir: 1 },
      { angle: -90, dir: 2 }, { angle: -135, dir: 3 },
      { angle: 180, dir: 4 }, { angle: -180, dir: 4 },
      { angle: 135, dir: 5 }, { angle: 90, dir: 6 },
      { angle: 45, dir: 7 },
    ];
    let bestDir = 0, minDiff = 360;
  ...
  getBestActions(aiPlayer, humanPlayer, players, asteroids) {
    ...
    const addAction = (action) => {
      const type = action.includes("turn") ? "turn" : action.includes("celerate") ? "speed" : "fire";
    ...
    if (humanShot && humanShot.type === "player" && humanShot.object.id === aiPlayer.id) {
      addAction("turnRight");
      if (aiPlayer.speed < aiPlayer.hp && aiPlayer.speed < 6) addAction("accelerate");
    }
